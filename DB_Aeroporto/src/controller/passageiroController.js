    ...
    checkInPassageiro: async(request, response) => {
        ...
        try {
            ...
            if (voo.status !== 'Embarque') {
                return res.status(400).json({ error: 'O check-in só pode ser realizado se o status do voo for "Embarque".' });
            }
            ...
            return res.status(200).json({ message: 'Check-in realizado com sucesso!', passageiro });
        } catch (error) {
